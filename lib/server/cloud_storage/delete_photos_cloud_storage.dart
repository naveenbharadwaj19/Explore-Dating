...
Future<void> deleteBodyPhotoFromCloudStorage(
  ...
  try {
    ...
    print("Error in deleting photo from storage : ${error.toString()}");
    Flushbar(
      messageText: Text(
...
Future<void> deleteChatPhotosFromCloudStorage(
  ...
  try {
    ...
    String optimizeUrlPath = Uri.decodeFull(path.basename(url)).replaceAll(new RegExp(r'(\?alt).*'), ""); // remove unwanted objects from the url
    await _storage.ref().child(optimizeUrlPath).delete();
    print("Chat photo deleted from storage");
  } catch (error) {
    print("Error in deleting chat photo from storage : ${error.toString()}");
  }
}
